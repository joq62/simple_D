...
%%% Created : 10 dec 2012
%%% -------------------------------------------------------------------
-module(etcd_service_test_cases). 
...
start()->
    application:start(etcd_service),
    ?assertEqual(ok,node_info_test1()),
...
%% -------------------------------------------------------------------
node_info_test1()->
    VmName=vm_name,Vm=vm,IpAddr=ipaddr,Port=port,Mode=mode,Status=status,
    etcd_service:update_node_info(VmName,Vm,IpAddr,Port,Mode,Status), 
    ?assertEqual([{{node_info,vm_name,vm,ipaddr,port},
		   {node_info,vm_name,vm,ipaddr,port,mode,status}}],etcd_service:read_node_info(all)),

    ?assertEqual([{{node_info,vm_name,vm,ipaddr,port},{mode,status}}],etcd_service:read_node_info(VmName)),
    ?assertEqual([],etcd_service:read_node_info(glurk)),
